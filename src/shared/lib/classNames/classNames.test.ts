...
describe('classNames', () => {
    ...
    test('Additional mods to params test', () => {
        ...
        expect(
            classNames('class', { hovered: true, scrollable: true }, ['class1', 'class2']),
        ).toBe(expected);
    });
    test('Additional false mods to params test', () => {
        const expected = 'class class1 class2 hovered';
        expect(
            classNames('class', { hovered: true, scrollable: false }, ['class1', 'class2']),
        ).toBe(expected);
    });
    test('Additional undefined mods to params test', () => {
        const expected = 'class class1 class2 scrollable';
        expect(
            classNames('class', { hovered: undefined, scrollable: true }, ['class1', 'class2']),
        ).toBe(expected);
    });
});
